...
Names in `monospace` — `tree_eq`, `archive/live-kernel/kernel/…`, `OPTIMIZER.typ` — are signposts into the disp codebase and docs for contributors. A general reader can ignore them; the prose is written to stand on its own.
