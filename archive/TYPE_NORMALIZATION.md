...
The second column is the **decidability frontier** — the same line `research/OPTIMIZER.typ`
...
- **The decidability frontier.** Structural = free (normalize); deep = proof (transport). This is the
  set-level-intensional + OTT/licensing stance of `research/OPTIMIZER.typ`.
...
- Companions: `NEGATIVE_TYPES.md` (telescope shape, §4b `qid`/Intersection, the polarity boundary),
  ...
  `research/OPTIMIZER.typ` (the decidability frontier / transport).
