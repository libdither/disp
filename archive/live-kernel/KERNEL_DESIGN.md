...
fragments `archive/live-kernel/kernel/{cut,engine,cells,base,positive,generic,universe}.disp`
(assembled in value-dependency order by `archive/live-kernel/kernel/prelude.disp`):
...
`archive/live-kernel/std/build.disp`) fills them from the recipes (it takes the
...
`Err`. These are pinned by `archive/live-kernel/tests/soundness.test.disp`.
...
used by shims such as `archive/live-kernel/kernel/prelude.disp`. Top-level redefinition
...
equation marker get the same pristine-capture treatment). Policies live in `archive/live-kernel/std/relation.disp`: `license_guard R`
