...
> Archived 2026-08-17: written against the retired live kernel (archive/live-kernel/).
...
two-horns pin in `archive/live-kernel/tests/soundness.test.disp` shows the boundary is one line
...
The behavioral half of the tower is running:
  ...
  - Every kernel fragment's typed exports verify through the kernel at load
    (`verify` pins in `archive/live-kernel/tests/use_raw.test.disp`); the standard library and the
