...
- **In disp.** `Telescope` is the one negative former; `Pi`/`Sigma`/`Record`/`⊤` are instances differing only in the cell op (`mint`/`apply`/`proj`/`deriv`); a single mode-polymorphic walker `at` serves both recognition and response (`archive/live-kernel/NEGATIVE_TYPES.md`, `types.disp`). Recursion and corecursion are *also* cells on the same walker (`TELESCOPE_FIXPOINT.md`); `Mu`/`Nu` wrappers were retired.
