...
fn main() {
    ...
    write_bundle(&path, scenarios);

    // The frontier tier, a SECOND file regenerated with every bundle so the picker
    // never trails the suite: the deep-reduction corpus frontier_deep_reductions pins
    // (parked scenarios replay to their parked end state — the PARKED badge is the
    // point) plus one census family member per congestion class that recently moved.
    let frontier_path = path.replace("lattice_cascade", "lattice_frontier");
    assert_ne!(frontier_path, path, "bundle path must contain 'lattice_cascade'");
    let k = rust_ca_lattice::oracle::k();
    let frontier: Vec<(String, String, Runner, u64)> = vec![
        ...
        (
            "frontier-k-comb".into(),
            "@(K,S(L),L) row-embedded: completes through triage with erasure of the \
             discarded arm"
                .into(),
            term_runner(&ap(ap(k.clone(), s(Term::L)), Term::L)),
            20_000_000,
        ),
        (
    ...
    write_bundle(&frontier_path, frontier);
}

fn write_bundle(path: &str, scenarios: Vec<(String, String, Runner, u64)>) {
