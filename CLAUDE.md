...
- `lib/kernel.disp` — unified selector-query kernel, type-tracking neutrals, infer, conversion, primitive type constructors, eliminators, and arithmetic.
...
- **Full kernel pipeline as `.disp` source.** Split into `lib/prelude.disp` (combinators) and `lib/kernel.disp` (unified selector-query kernel and type surface). Files use `open use "dep.disp"` for dependencies. Types are `wait(checker)(metadata)` — type checking is raw `apply(T, v) = TT`. Type-former tags have been removed.
...
- **Smart accum (type-tracking neutrals).** Neutrals use a smart `accum` handler that tracks types through accumulation. VHyp = `wait(accum)(fork(type, fork(HYP_TAG, id)))` — type stored at `pair_fst` of metadata. When a Pi-typed neutral is applied, smart accum computes `codFn(v)` as the result type. `infer` is O(1): just extracts the stored type. `is_neutral` is a single O(1) check: `fast_eq(pair_fst(x), NEUTRAL_SIG)`. No val_apply or type_apply — raw `apply` handles types, neutrals, and functions uniformly.
...
`npm test` runs vitest. `lib/*.test.disp` is the primary test suite (146 tests across 2 files). `test/parser.test.ts` (62 tests) and `test/tree.test.ts` (21 tests) cover the host infrastructure.
