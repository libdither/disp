...
- ✅ **Ord library** (`lib/ord.disp`, V2 §4.8) — DAE-encoded ordinals up to ε₀ via Cantor normal form. Constructors `zero_ord` / `omega_plus`, eliminator `ord_rec`, derived `succ_ord` / `omega`. Hypothesis-Pi-typed via `mint_ord_hyp`. 15 tests.
- ⏳ Full drop of kernel.bool/nat/eq fields from the recq record requires migrating Bool/Nat/Eq to Pi-shape (per-value templates), which needs Scott constructors. Plan documented in task #3.
- ⏳ `ord_lt` / `ord_le` / `ord_max` kernel comparison primitives + `OrdLt k` refinement. Kernel handlers traverse Scott-encoded Ord values; bound-consulting identities for hypothesis-typed Ord (V2 §5.5). Re-base universe ranks from Nat to Ord in q_core_type_fn / q_guarded_type_fn.
