...
// ── native CLI entry points (M2d) ────────────────────────────────────────────
// The parallel reducer has no in-process host (crossbeam/threads don't build on wasm —
// that needs napi). Instead a thin native bin (`src/bin/ic-net-cli.rs`) drives these as a
// cold-subprocess benchmark contestant, mirroring bench/disp-cli.ts: NF on stdout,
// `reduce_ms` on stderr, the timer excluding the (fixed) arena allocation. Both return
// `(nf, reduce_ms, interactions)`, or None on budget exhaustion.
#[cfg(not(target_arch = "wasm32"))]
