...
|---|---|---|---|
| G1 Substrate | ◐ 50% (Lean meta) | Inherits Lean metaprogramming; Loom generates verifiers *as* Lean developments, which is a real reflective move. | 1 · 0 · ½(metaprograms) — Lean metaprogramming; Loom generates verifiers as Lean developments |
| G2 Specification | ◐ 65% (Lean+mathlib) | Full Lean + mathlib behind a Dafny-style surface. Equality: Lean's, plus SMT within its fragment. Not a rewrite-licensing relation. | 1 · 1 · 0 · ½(SMT-fragment) — Lean + mathlib behind a Dafny-style surface; Lean's equality plus SMT in its fragment |
| G3 Trust | ✅ 80% (Lean kernel) | Lean's kernel; Loom verifiers are *foundational* (proofs bottom out in Lean, not in a trusted VC generator). | ½(mid-size kernel) · 1 · 1 · — — Lean's kernel; Loom verifiers are foundational, no trusted VC generator |
| G4 Execution | ✗ 0% (none) | Imperative programs extracted for testing; no native systems backend, no cost model. | 0 · 0 · 0 — programs are extracted for testing only |
| G5 Search | ◐ 60% (agentic loop) | Best-documented agentic verified-synthesis loop with published benchmark numbers. | 1 · ½(checker only) · 0 — WybeCoder's agentic loop proves 74% of Verina; correctness only, no cost |
