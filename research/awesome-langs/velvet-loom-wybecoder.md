...
|---|---|---|---|
| A1 Reflection | ◐ 50% (Lean meta) | Inherits Lean metaprogramming; Loom generates verifiers *as* Lean developments, which is a real reflective move. | 1 · 0 · ½ — Lean metaprogramming; Loom generates verifiers as Lean developments |
| A2 Spec power | ✅ 100% (Lean+mathlib) | Full Lean + mathlib behind a Dafny-style surface. | 1 · 1 · 1 — Lean + mathlib behind a Dafny-style surface |
| A3 Kernel | ✅ 83% (Lean kernel) | Lean's kernel; Loom verifiers are *foundational* (proofs bottom out in Lean, not in a trusted VC generator). | ½ · 1 · 1 — Lean's kernel; Loom verifiers are foundational, no trusted VC generator |
| A4 Equality | ◐ 33% (Lean+SMT) | Lean's, plus SMT within its fragment. Not a rewrite-licensing relation. | ½ · ½ · 0 — Lean's equality plus SMT in its fragment |
| A5 Perf | ✗ 0% (none) | Imperative programs extracted for testing; no native systems backend, no cost model. | 0 · 0 · 0 — programs are extracted for testing only |
| A6 Search | ◐ 50% (agentic loop) | Best-documented agentic verified-synthesis loop with published benchmark numbers. | 1 · ½ · 0 — WybeCoder's agentic loop proves 74% of Verina; correctness only, no cost |
