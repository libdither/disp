...
- ⏳ 5 polymorphic Pi tests still fail (`Pi Type0 ({A} -> Arrow A A)` and similar). Arrow's body calls `unguard_checked` on a hypothesis-typed A, which the walker rejects. Needs walker-safe Pi/Arrow rewrite or kernel handler for `unguard_checked`.
