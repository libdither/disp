...
| G2 Specification | ✗ 15% (comptime values) | Compile-time-value indexing, `where` clauses, conditional conformance, linear types. No propositions, no proofs, no runtime-value dependency. Equality: MLIR rewrites are unverified compiler transforms; no semantic licensing, no certificates. | ½(comptime values) · 0 · 0 · 0 — types indexed by compile-time values only; no propositions, no proofs, no equality |
| G3 Trust | ✗ 12% (MLIR stack) | Trust is the whole MLIR/LLVM C++ stack. Now visible, never small; its rewrites are asserted transforms. | 0 · 0 · 0 · ½(asserted) — the whole MLIR/LLVM C++ stack; MLIR rewrites are asserted compiler transforms, unverified |
| G4 Execution | ◐ 40% (MLIR codegen) | The strongest hardware story in this survey: MLIR codegen for CPU/GPU/accelerators, vendor-class kernels, zero-cost flat structs. | 1 · 0 · 0 — MLIR codegen for CPU/GPU/accelerators |
| G5 Search | ◐ 40% (param sweeps) | Offline benchmark sweeps over kernel parameter grids, cost only, no checker in the loop; the in-language `autotune` was removed in v0.7.0. | ½(sweeps) · ½(cost only) · 0 — offline parameter sweeps, cost only, correctness assumed |
