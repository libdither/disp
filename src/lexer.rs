...
use winnow::{
	...
	combinator::{alt, dispatch, fail, peek, preceded, repeat, separated_pair, terminated},
...
fn string<'i, E: ParserError<&'i str> + AddContext<&'i str, StrContext>>(input: &mut &'i str) -> PResult<String, E> {
	preceded(
		'\"',
		terminated(
...
fn test_lexer_string() {
	assert_eq!(
		token.parse_peek(r#""yeetaloni"3"#),
		Ok(("3", Token::String("yeetaloni".to_owned())))
	);
}
#[test]
fn test_lexer_ops() {
...
fn test_lexer_tokenstream() {
	...
	assert_eq!(
		lexer(&mut "(abcabc)"),
		Ok(vec![OpenParen, Ident("abcabc".to_owned()), ClosedParen])
	);

	assert_eq!(
		lexer(&mut r#"a := "thing""#),
		Ok(vec![Token::Ident("a".to_owned()), AssignOp, String("thing".to_owned())])
	);
}
