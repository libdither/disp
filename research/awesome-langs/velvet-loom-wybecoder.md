...
| G2 Specification | ◐ 62% (Lean+mathlib) | Full Lean + mathlib behind a Dafny-style surface. Equality: Lean's, plus SMT within its fragment. Not a rewrite-licensing relation. | 1 · 1 · 0 · ½(SMT-fragment) — Lean + mathlib behind a Dafny-style surface; Lean's equality plus SMT in its fragment |
| G3 Trust | ✅ 83% (Lean kernel) | Lean's kernel; Loom verifiers are *foundational* (proofs bottom out in Lean, not in a trusted VC generator). | ½(mid-size kernel) · 1 · 1 — Lean's kernel; Loom verifiers are foundational, no trusted VC generator |
...
| G5 Search | ◐ 50% (agentic loop) | Best-documented agentic verified-synthesis loop with published benchmark numbers. | 1 · ½(checker only) · 0 — WybeCoder's agentic loop proves 74% of Verina; correctness only, no cost |
