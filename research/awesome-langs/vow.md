...
| G3 Trust | ✗ 0% (BMC TCB) | TCB = vowc + ESBMC. No certificates, no LCF split. | 0 · 0 · 0 · 0 — TCB = vowc + ESBMC |
| G4 Execution | ◐ 40% (Cranelift) | Cranelift native codegen, linear types, self-hosted with byte-identical bootstrap. Real systems performance. | 1 · 0 · 0 — Cranelift native with a byte-identical bootstrap |
| G5 Search | ◐ 40% (CEGIS+LLM) | CEGIS with an external LLM as proposer. No internal search, no cost objective, no self-optimization. | ½(LLM loop) · ½(checker only) · 0 — CEGIS with an external LLM; no cost objective |
...
**Distance from disp's goals: shares the motivation and G4; opposite on G1;
weaker G2/G3; G5 outsourced to an LLM.**
