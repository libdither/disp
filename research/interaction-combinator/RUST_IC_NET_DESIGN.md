...
- **M2c PERF — ⚠ the parallel schedule REGRESSES (M2d benchmark, 12-core box).** The earlier "~3.1× at depth 8" claim does NOT reproduce; on a clean thread-sweep, 2t/4t/8t are **3–30× SLOWER than the 1t sequential drain** at every size (wide d12×c8: 1t=5.0ms, 2t=35ms, 4t=41ms, 8t=36ms). Cause: the schedule does **per-interaction global atomics** — `budget.fetch_sub` + `interactions.fetch_add` + an **AcqRel** `in_flight` RMW, all on shared cache lines *every* interaction — plus the single shared `Injector`. That serializes + cache-bounces faster than parallelism recovers. **Fix = the §6 scheduler (deferred, now REQUIRED for any speedup): per-worker work-stealing deques + thread-local budget/interaction counters synced periodically + a generation/quiescence termination handshake instead of per-interaction `in_flight`.** Until then ic-net's best config is sequential.
