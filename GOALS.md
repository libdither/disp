...
*(Status 2026-07-11: the licensing half of the loop exists in miniature — explicit compositional relation objects (`lib/std/relation.disp`), machine-checked rewrite licenses proven by induction, and guard-owned names whose redefinition demands relation evidence re-verified at every load (the declaration protocol: SYNTAX.typ § record members, `lib/kernel/cut.disp`). Nat and `case_value` state their optimizer contracts directly; observer-restricted certification remains open. The search/scoring half — the external optimizer — remains.)*
