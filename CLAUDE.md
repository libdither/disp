...
- ✅ Bool/Nat/Eq dropped from V2 dispatcher signature list. Their predicate applications no longer have native fast-path routing — fall through to the walker. Recq fields stay (q_core_type_fn uses them internally for is_registered).
- ✅ **Ord library** (`lib/ord.disp`, V2 §4.8) — DAE-encoded ordinals up to ε₀ via Cantor normal form. Constructors `zero_ord` / `omega_plus`, eliminator `ord_rec`, derived `succ_ord` / `omega`. Hypothesis-Pi-typed via `mint_ord_hyp`. Closed comparisons `ord_lt` / `ord_le` / `ord_max` via Scott eliminator probing. 31 tests.
- ⏳ Full drop of kernel.bool/nat/eq fields from the recq record (the Scott constructor migration). Requires choosing canonical "true" representation (LEAF marker vs Scott — V2 is ambiguous) and rewriting prelude `select`/`select_lazy`, `src/tree.ts`'s `tree_eq` fast-path, and ~80 test sites. Coherent 3-5 hour follow-up session.
- ⏳ Kernel-routed `ord_lt` / `ord_le` / `ord_max` for hypothesis-typed Ord (V2 §5.5 bound-consulting). `OrdLt k` refinement type. Re-base universe ranks from Nat to Ord in q_core_type_fn / q_guarded_type_fn.
