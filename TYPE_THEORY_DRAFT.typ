#set document(title: "Disp Type Theory — Unified Design Draft")
...
#align(center, text(22pt, weight: "bold")[Disp Type Theory — Unified Design Draft])
#v(0.3em)
#align(center)[
  Proposed direction. Distinct from current implementation (`lib/kernel.disp`)
  and from `TYPE_THEORY.typ` (the data-as-eliminator design).\
  This document supersedes neither until explicitly adopted.
]
#v(1em)

#note[
  *Status.* Draft. Captures the kernel design where:
  ...
  hypothesis is reachable via a non-neutral path. This subsumes
  earlier proposals that restricted bind_hyp's body to return Bool.
  See §4.6 for the bind_hyp specification and §9 for the soundness
  argument.
]
