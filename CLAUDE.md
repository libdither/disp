...
- `kernel/` — the two-Σ-op kernel center, split into three fragments assembled by `prelude.disp`:
  ...
  - `types.disp` — **library types + structural typedefs:** Bool, Nat, Pi/Arrow, Type (+ `type_predicate_h_rule` H-rule), Unit, False/Not, String, Eq (+ `eq_J`/`eq_subst`/`eq_sym`/`eq_cong`), Ord, Refinement, Intersection, Coproduct — each `wait (make_recognizer body) meta` with the recognizer body inline. **`Telescope`** — the dependent n-ary record former (entry = `{name; ty; def}`, def a stem-option: `t` opaque / `t e` derived-pinned-by-`tree_eq`; tail = `λx. rest`) with `tele_check` (concrete feed) / `tele_respond` (proj-neutral feed; opaque → `Extend ty`, derived → `Reduce e`) and the builder `mk T given` (fills derived fields; takes the TYPE). `Record` (lazily-lifted constant-tail telescope) and `Sigma` (`{ fst : A, snd : B fst }`; values are records, not fork-pairs) are telescope instances. Recursors `nat_rec`/`bool_rec`/`ord_rec` via `elim`. The kernel's own structural types `Tree`/`Frame`/`NeutralMeta`/`Action`/`RespondShape`/`MetaShape`. See `KERNEL_DESIGN.md` § Telescopes.
- `elab/` — the self-hosting elaborator, piece by piece (roadmap: `ELABORATOR_PLAN.md`): `bracket.disp` — in-language bracket abstraction (Cir coproduct `Lit|Var|App|Lam|S|K|I`; `contains_free`/`abstract_name` with the η, S(Kp)I, and K-composition optimizations / `eliminate_lams` / `cir_to_tree` / `bracket_compile`). Bracket abstraction defines which tree a binder becomes — i.e. it is part of definitional equality — so this is the SPEC and the host (`src/compile.ts`) is the fast path, validated bit-identically by `lib/tests/bracket.test.disp` (in-language vs host-compiled lambdas) and `test/bracket.test.ts` (300 randomized closed terms).
