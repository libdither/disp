...
- **Type-tracking neutrals.** `hyp_reduce` tracks types through accumulation: if the neutral's type is Pi, result_type = codFn(v); otherwise FF. Metadata: `fork(type, payload)`. `neutral_type(v) = pair_fst(type_meta(v))`. `infer` is O(1) extraction, not spine walking. `is_neutral = has_sig kernel.hyp_reduce v` is O(1). No val_apply or type_apply needed.
