...
- [`TYPE_THEORY.typ`](TYPE_THEORY.typ) — **authoritative** type-theory spec (target). Three Σ-operations (`hyp_reduce`, `bind_hyp`, `postulate`) plus a Σ-parameterized dispatcher (`safe_apply`) that routes pinned sigs to the *registered* handler (Σ is a routing table, not just a trust set — `safe_apply Σ f x` runs `h (wait_meta f) x` for the registered `h`, ignoring `f`'s embedded handler). `eliminator_frame` is demoted to a library `elim` over `hyp_reduce` + a type's `respond` meta-field. Walker protection splits three ways: `seal(Σ)` (trusted-token producers — `hyp_reduce`; unforgeable *and* uninspectable), `funnel(Σ)` (host sigs forced through `postulate`'s sanitizer — unforgeable only), and `forge(Σ) = seal ∪ funnel`; `bind_hyp`/`postulate` are in neither (their invocations are built by library recognizers under the walker). Manifest contracts over the tree-calculus substrate; `CheckerResult` monad with named-variant errors; library types carry MetaShape-conforming meta records (`recognizer_params`, `functor`, `respond`, `behavioral_specs`); cubical operations live in each type's `functor` meta-field; validators (`Type` / `StrictType` / `BehavioralType`) are ordinary library wait-forms.
...
**The codebase implements the seven-primitive kernel shape from `TYPE_THEORY_LEGACY.typ`.** Pi, Bool, Nat, Eq, Type, and Ord are ordinary library types under `lib/types/`; the kernel no longer has per-type handlers for them. The new `TYPE_THEORY.typ` retargets to a three-Σ-operation kernel (`hyp_reduce`, `bind_hyp`, `postulate`) plus a Σ-parameterized routing-table dispatcher (`safe_apply`); `eliminator_frame` folds into `hyp_reduce` + a library `elim`. Migration is open work and no `lib/kernel/` files have moved to the new shape yet.
