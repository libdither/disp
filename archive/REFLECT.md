...
> Archived 2026-08-17: written against the retired live kernel; its prototype and pins live under archive/live-kernel/tests/.

Status: prototype landed 2026-07-07 in `archive/live-kernel/tests/reflect_proto.test.disp` (23 pins),
building on the two-horns characterization in `archive/live-kernel/tests/soundness.test.disp` and the
free-monad effects layer in `archive/live-kernel/std/effect.disp` (TYPE_THEORY §15). This note records
...
## Files
  ...
  - `archive/live-kernel/std/effect.disp`: the free monad, handlers, rows, `EffAt`.
  - `archive/live-kernel/tests/reflect_proto.test.disp`: the Reflect signature, the floor handler,
    the twins, the modality pins, `walk_spec` and its differential battery.
  - `archive/live-kernel/tests/soundness.test.disp`: the two-horns characterization this answers,
    and the K0 floor pins it lives next to.
  - `archive/live-kernel/tests/tree_rec.test.disp`: the eliminative face (typed twins over concrete
    shape recursion).
