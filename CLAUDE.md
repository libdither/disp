...
**The code is the status, not this file.** For what's *landed*, read `git log`, `npm test`, and the kernel/std sources — the architecture is described under "Code layout" above; don't maintain a duplicate checklist here (it rots on every commit). For what's *not yet built*, the authoritative tracker is `TYPE_THEORY.typ` (by § number) plus the characterization tests in `lib/tests/soundness.test.disp`. Known-deferred at a glance: route-to-registered handler (§5.4); `StrictType` / `BehavioralType` + `RespondShape` / `RecognizerShape` and a stricter `Type` (§11–§12); effects `Eff R X` + driver (§15); cubical `Path` / `comp` / `transp` / `Glue` (§13); `strip` / erasure (§10); `wf_fix` / `Total`; richer `CheckerError` vocabulary + source-span diagnostics (currently one `Err`). **Landed (was deferred):** `bind_hyp` body-walking (§7.2) — `bind_hyp` is merged into `param_walker` (`w_bind_hyp`), so the body runs UNDER the walker via the walker's own `self`; raw reflection in a body is rejected (the metadata-extraction leak is closed). The escape-only `occurs` is no longer the sole guard. Module typed exports auto-verify through the kernel at elaboration time (`verify mod` = `param_apply typ record`); raw recognizer application (`typ record`) bypasses the walker, so verification must go through `param_apply`/`verify`. A binder type (Pi/Intersection/Sigma) applied raw fails fast (its `bind_hyp` can't run outside the walker).
