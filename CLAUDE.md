...
**The codebase implements the seven-primitive kernel shape from `TYPE_THEORY_LEGACY.typ`.** Pi, Bool, Nat, Eq, Type, and Ord are ordinary library types under `lib/types/`; the kernel no longer has per-type handlers for them. The new `TYPE_THEORY.typ` retargets to a **two**-Σ-operation kernel (`hyp_reduce`, `bind_hyp`) plus the two-argument routing-table dispatcher `param_apply` over a fixed Σ; `eliminator_frame` folds into `hyp_reduce` + a library `elim`, and `postulate` is removed in favor of a library `Eff R X` free-monad effect system + driver (§15). Migration is open work and no `lib/kernel/` files have moved to the new shape yet.
