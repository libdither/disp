...
//! open addressing) — no per-entry deserialization or rehash on load.
//!
//! Layout (little-endian, sections 8-byte-aligned):
//!   magic u64 | version u64 | stamp_len u64 | node_len u64 | intern_cap u64 |
//!   memo_cap u64 | stamp (padded to 8) | nodes u64×node_len |
...
//! the reserved null sentinel, never a valid key). Both tables are power-of-two
//! capacity at ≤ 0.5 load, linear probing.
...
/// ids (the base occupies `0..node_len`).
...
impl FrozenBase {
    ...
    /// Iterate frozen memo entries (for re-saving a grown snapshot).
    pub(crate) fn memo_iter(&self) -> impl Iterator<Item = (u32, u32, u32)> + '_ {
        (0..self.memo_f.len()).filter_map(move |i| {
            let f = self.memo_f[i];
            (f != 0).then(|| (f, self.memo_x[i], self.memo_r[i]))
        })
    }
    /// Iterate frozen interned ids (for re-saving a grown snapshot).
...
fn read_u64s(f: &mut File, n: usize) -> std::io::Result<Vec<u64>> {
    ...
    // Read straight into the Vec<u64>'s bytes: aligned by construction, one copy
    // from the kernel. Little-endian layout (x86-64 / aarch64 — the addon's hosts).
...
fn write_u64s(w: &mut impl Write, v: &[u64]) -> std::io::Result<()> {
    w.write_all(unsafe { std::slice::from_raw_parts(v.as_ptr() as *const u8, v.len() * 8) })
}
fn write_u32s(w: &mut impl Write, v: &[u32]) -> std::io::Result<()> {
    w.write_all(unsafe { std::slice::from_raw_parts(v.as_ptr() as *const u8, v.len() * 4) })
}

pub(crate) fn load(path: &str, stamp: &[u8]) -> Result<FrozenBase, String> {
    let mut f = File::open(path).map_err(|e| e.to_string())?;
    let header = read_u64s(&mut f, 6).map_err(|e| e.to_string())?;
    if header[0] != MAGIC || header[1] != VERSION {
        return Err("bad magic/version".into());
    }
    let stamp_len = header[2] as usize;
    ...
    let (node_len, intern_cap, memo_cap) = (header[3] as usize, header[4] as usize, header[5] as usize);
    if node_len >= (1 << 31) {
    ...
    Ok(FrozenBase { nodes, intern, memo_f, memo_x, memo_r })
}
...
/// Write a snapshot: the full node table plus intern/memo tables built from the
/// given entry iterators. Atomic via temp + rename.
pub(crate) fn save(
    path: &str,
    stamp: &[u8],
    nodes: Vec<u64>,
    intern_ids: impl Iterator<Item = u32>,
    memo_entries: impl Iterator<Item = (u32, u32, u32)>,
    ...
    let entries: Vec<(u32, u32, u32)> = memo_entries.collect();
    let mcap = table_cap(entries.len());
    let mmask = mcap - 1;
    let (mut mf, mut mx, mut mr) = (vec![0u32; mcap], vec![0u32; mcap], vec![0u32; mcap]);
    let mut memo_count = 0u32;
    for (f, x, r) in entries {
    ...
    let tmp = format!("{path}.tmp");
    {
        ...
        write_u64s(&mut w, &[MAGIC, VERSION, stamp.len() as u64, nodes.len() as u64, icap as u64, mcap as u64])
            .map_err(|e| e.to_string())?;
        w.write_all(stamp).map_err(|e| e.to_string())?;
        w.write_all(&vec![0u8; padded - stamp.len()]).map_err(|e| e.to_string())?;
        write_u64s(&mut w, &nodes).map_err(|e| e.to_string())?;
        write_u32s(&mut w, &intern).map_err(|e| e.to_string())?;
        write_u32s(&mut w, &mf).map_err(|e| e.to_string())?;
        write_u32s(&mut w, &mx).map_err(|e| e.to_string())?;
        write_u32s(&mut w, &mr).map_err(|e| e.to_string())?;
