<script lang="ts">
  ...
  function layout(tree: T): { nodes: VNode[]; edges: VEdge[] } {
    ...
    for (const n of out) {
      ...
      for (let i = 0; i < kids.length; i++) {
        ...
        vedges.push({
          id: n.path + '>' + i,
          d: styled ? `M ${x1} ${y1} Q ${mx} ${my} ${x2} ${y2}` : `M ${x1} ${y1} L ${x2} ${y2}`,
  ...
  function show(tree: T) {
    ...
    const l = layout(tree)
    if (motion && styled && prev.size > 0) {
      // leaves whose paths vanished become debris: they fall to the ground,
      // rest there, and fade on their own schedule
      const kept = new Set(l.nodes.map((n) => n.path))
      const fresh: Debris[] = []
      for (const [path, n] of prev) {
        if (kept.has(path) || n.tag !== 'leaf') continue
        fresh.push({
          id: debrisId++,
          x: n.x,
          y: n.y,
          dy: height - GROUND + 4 - n.y + (hash(path) % 8),
          drift: ((hash(path) % 36) - 18) * 1.1,
          rot: (hash(path) % 160) - 80,
          tint: n.tint
        })
      }
      if (fresh.length) {
...
<div class="vis" class:plain={!styled} class:lab={variant === 'lab'}>
  ...
  {:else}
    <svg viewBox="0 0 {W} {height}" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
      ...
      <!-- fallen leaves: they land, they rest, they let go -->
      {#each debris as d (d.id)}
        <g style="transform: translate({d.x}px, {d.y}px)">
          <g class="debris" style="--dy: {d.dy}px; --dx: {d.drift}px">
            <path
      ...
      <g class="tree" class:swaying={styled && motion}>
        ...
        {#each edges as e (e.id)}
          <path d={e.d} class="branch" style="stroke-width:{e.w}" />
        {/each}
...
<style>
  ...
  /* ---- fallen leaves: fall (1s), rest (2.4s), let go (3.4s) ---- */
  .debris {
    animation:
