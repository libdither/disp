...
- `kernel/` — the two-Σ-op kernel center, split into three fragments assembled by `prelude.disp`:
  ...
  - `cut.disp` — **the §2.6 value cut:** `prod`/`annihilate`/`proj`/`inj`/`acc`/`mk_record`/`field` + `type_meta`/`checker_sig`; control coproducts `Action` (`Extend | Return`) / `CheckerResult` (`Ok | Err`); `make_meta` (MetaShape builder). Records (metadata, `recognizer_params`, neutral-meta `{stored_type; payload}`, eliminator frame `{motive; cases}`) are read **by name** through the cut (`.field`).
